...
GET /ws — WebSocket.
Endpoint Método Uso
/agata POST Recebe telemetria /agata/send-command POST Enfileira um comando para ser enviado ao próximo ciclo do device. /agata/queue/:serial GET Consulta se há comando pendente para um device específico. /agata/queue GET Lista todos os comandos pendentes para todos os devices. /agata/config/template GET Pegar estrutura padrão para formulário /agata/config/:serial POST Enviar nova configuração /agata/changes/:serial GET Histórico de alterações
